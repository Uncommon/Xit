...
{
  static let rootPath = TeamCityAPI.rootPath // reuse existing constant
  ...
  func buildTypesForRemote(_ remoteURLString: String) -> [String]
  {
    ...
    {
      remoteURL.host == url.host && remoteURL.path == url.path
    }

    let result = buildTypeURLs.keys.filter {
      buildTypeURLs[$0]?.contains(where: matchHostPath) ?? false
    }
  ...
  {
    let rootIDs = vcsRootsForBuildType(buildType)
    let displayNames = rootIDs.compactMap { vcsBranchSpecs[$0]?.match(branch: branch.fullPath) }

    return displayNames.reduce(nil) { shortest, name in
      (shortest?.count ?? Int.max) < name.count ? shortest : name
    }
  }
  ...
  private func parseBuildTypeDetail(from data: Data) throws
  {
    ...
    let urls = vcsIDs.compactMap { id in
      lock.lock(); defer { lock.unlock() }
      return vcsRootMap[id]
    }
