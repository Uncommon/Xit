...
{
  NSMutableString *table =
      [NSMutableString stringWithString:@"<td><table class='headercol'>"];
  NSString *firstLine = message;
  const NSRange lineEndRange =
      [message rangeOfCharacterFromSet:[NSCharacterSet newlineCharacterSet]];

  if (lineEndRange.location != NSNotFound)
    firstLine = [message substringToIndex:lineEndRange.location];

  void (^addRow)(NSString *, NSString *) =
      ^(NSString *label, NSString *content) {
    [table appendFormat:@"<tr><td>%@:</td><td>%@</td></tr>", label, content];
  };
  void (^addPerson)(NSString *, NSString *, NSString *, NSDate *) =
      ^(NSString *type, NSString *name, NSString *email, NSDate *date) {
  ...
  return [NSString stringWithFormat:@"<table class='header'><tr>%@</tr></table>"
                                     "<p class='subject'>%@</p>",
          table, message];
}
...
- (NSString *)htmlForFiles:(NSArray *)files
{
  ...
  for (NSString *file in files) {
    [html appendFormat:
        @"<p><a class='%@' href='#%@' representedfile='%@'>%@</a></p>",
        @"M", file, file, file];
  }
  ...
  NSString *html =
      [NSString stringWithFormat:
          @"<html><head><link rel='stylesheet' type='text/css' "
           "href='diff.css'/></head>"
           "<body>%@%@<div id='diffs'>%@</div></body></html>",
          headerHTML, filesHTML, diffHTML];
  ...
  for (NSString *line in lines) {
    if ([line length] == 0) {
      ...
      if (parsingSubject) {
        ...
        [result addObject:@{ kHeaderKeyName : @"subject",
                             kHeaderKeyContent : trimmedLine }];
      } else {
        NSArray *comps = [line componentsSeparatedByString:@" "];
        if ([comps count] == 2) {
          [result addObject:
                  @{ kHeaderKeyName : comps[0], kHeaderKeyContent : comps[1] }];
        } else if ([comps count] > 2) {
          NSRange r_email_i = [line rangeOfString:@"<"];
          NSRange r_email_e = [line rangeOfString:@">"];
          NSRange r_name_i = [line rangeOfString:@" "];

          NSString *name = [line substringWithRange:NSMakeRange(
              r_name_i.location, (r_email_i.location - r_name_i.location))];
          NSString *email = [line substringWithRange:
                  NSMakeRange(r_email_i.location + 1,
                              ((r_email_e.location - 1) - r_email_i.location))];

          NSDate *date = nil;

          if ([line length] > r_email_e.location + 2) {
            NSArray *t = [[line substringFromIndex:r_email_e.location + 2]
          ...
          [result addObject:
              @{ kHeaderKeyName:comps[0], kHeaderKeyContent:content }];
        }
      }
    }
  }
...
- (NSString *)htmlForHeader:(NSArray *)header
{
  ...
  for (NSDictionary *item in header) {
    if ([item[kHeaderKeyName] isEqualToString:@"subject"]) {
      [subject appendString:[NSString stringWithFormat:@"%@<br/>",
                             [XTHTML escapeHTML:item[kHeaderKeyContent]]]];
    } else {
      if ([item[kHeaderKeyContent] isKindOfClass:[NSString class]]) {
        [refs appendString:[NSString stringWithFormat:
            @"<tr><td>%@</td><td><a href='' "
             "onclick='selectCommit(this.innerHTML); return false;'>%@"
             "</a></td></tr>",
            item[kHeaderKeyName], item[kHeaderKeyContent]]];
        ...
        if (![email isEqualToString:last_mail]) {
          ...
          [auths appendString:
                  [NSString stringWithFormat:@"<div class='user %@ clearfix'>",
                      item[kHeaderKeyName]]];
          [auths appendString:[
                  NSString stringWithFormat:
                      @"<p class='name'>%@ <span class='rol'>(%@)</span></p>",
                  name, item[kHeaderKeyName]]];
          [auths appendString:
                  [NSString stringWithFormat:@"<p class='time'>%@</p></div>",
                      dateString]];
        }
        last_mail = email;
      }
  ...
  return [NSString stringWithFormat:
      @"<div id='header' class='clearfix'><table class='references'>%@"
       "</table><p class='subject'>%@</p>%@</div>",
      refs, subject, auths];
}
                  ...
                  withStats:(NSMutableDictionary *)stats
{
  ...
  for (__strong NSString *line in lines) {
    ...
    [res appendString:[NSString stringWithFormat:
        @"<a class='%@' href='#%@' representedFile='%@'>%@</a>",
        status, file, fileName, txt]];
    [res appendString:@"</td><td class='bar'>"];
    [res appendString:@"<div>"];
    [res appendString:[NSString stringWithFormat:
        @"<span class='add' style='width:%d%%'></span>",
        (int)((add *100) / granTotal)]];
    [res appendString:[NSString stringWithFormat:
        @"<span class='rem' style='width:%d%%'></span>",
        (int)((rem *100) / granTotal)]];
    [res appendString:@"</div>"];
    [res appendString:[NSString stringWithFormat:
        @"</td><td class='add'>+ %d</td><td class='rem'>- %d</td></tr>",
        (int) add, (int) rem]];
  }
