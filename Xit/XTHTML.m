...
+ (NSString *)parseDiff:(NSString *)txt
{
  ...
  while ([scan scanString:@"diff --" intoString:NULL]) {  // is a diff start?
    ...
    [res appendString:[self
        parseDiffBlock:[NSString stringWithFormat:@"diff --%@", block]]];
  }
...
+ (NSString *)parseDiffBlock:(NSString *)txt
{
  ...
  while ([scan scanString:@"@@" intoString:NULL]) {
    [scan scanUpToString:@"\n@@" intoString:&block];
    [res appendString:
            [XTHTML parseDiffChunk:[NSString stringWithFormat:@"@@%@", block]]];
  }
...
+ (NSString *)parseDiffHeader:(NSString *)txt
{
  ...
  [res appendString:
          [NSString stringWithFormat:
                  @"<tr id='%@'><td colspan='33'><div style='float:left;'>",
              fileName]];
  do {
...
+ (NSString *)escapeHTML:(NSString *)txt
{
  ...
  [newTxt replaceOccurrencesOfString:@"&"
                          withString:@"&amp;"
                             options:NSLiteralSearch
                               range:NSMakeRange(0, [newTxt length])];
  [newTxt replaceOccurrencesOfString:@"<"
                          withString:@"&lt;"
                             options:NSLiteralSearch
                               range:NSMakeRange(0, [newTxt length])];
...
+ (NSString *)parseBinaryDiff:(NSString *)txt
{
  ...
  if (![files[0] isAbsolutePath]) {
    if ([XTHTML isImage:files[0]]) {
      [res appendString:
              [NSString stringWithFormat:@"<img src='GitX://{SHA}:/prev/%@'/>",
                                         files[0]]];
    }
  }
  ...
  if (![files[1] isAbsolutePath]) {
    if ([XTHTML isImage:files[1]]) {
      [res appendString:
              [NSString stringWithFormat:@"<img src='GitX://{SHA}:/%@'/>",
                                         files[1]]];
    }
  }
...
+ (NSString *)parseDiffChunk:(NSString *)txt
{
  ...
  NSRange hr =
      NSMakeRange(arity + 1, [line rangeOfString:@" @@"].location - arity - 1);
  ...
  [res appendString:[NSString stringWithFormat:@"<tr class='header'>"
                                                "<td colspan='%d'>%@</td></tr>",
                                               arity + 1, line]];
  while ((line = [lines nextObject])) {
    if ([line length] > 0) {
      ...
      if (![prefix hasPrefix:@"\\"]) {
        [res appendString:[NSString stringWithFormat:
            @"<td class='code'>%@</td></tr>",
            [line substringFromIndex:arity - 1]]];
      }
    }
  }
  free(l_line);
...
+ (BOOL)isImage:(NSString *)file
{
  ...
  return (mimeType != nil) &&
         ([mimeType rangeOfString:@"image/" options:NSCaseInsensitiveSearch]
              .location != NSNotFound);
}
...
+ (NSString *)mimeTypeForFileName:(NSString *)name
{
  ...
  if (i != NSNotFound) {
    ...
    CFStringRef UTI = UTTypeCreatePreferredIdentifierForTag(
        kUTTagClassFilenameExtension, (__bridge CFStringRef) ext, NULL);
    if (UTI) {
      CFStringRef registeredType =
