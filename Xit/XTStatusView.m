...
- (id)initWithFrame:(NSRect)frame
{
  ...
  if (self != nil) {
    NSColor *backgroundColor = [NSColor colorWithDeviceHue:212 / 360.0
                                                saturation:0.06
                                                brightness:0.8
                                                     alpha:1.0];
...
- (void)drawRect:(NSRect)dirtyRect
{
  ...
  const NSRect pathBounds = {bounds.origin,
                             {bounds.size.width, bounds.size.height - 1}};
  NSBezierPath *path = [NSBezierPath bezierPathWithRoundedRect:pathBounds
  ...
  NSBezierPath *framePath =
      [NSBezierPath bezierPathWithRoundedRect:pathFrameBounds
                                      xRadius:kCornerRadius
                                      yRadius:kCornerRadius];

  [framePath appendBezierPathWithRoundedRect:NSInsetRect(pathFrameBounds, 1, 1)
                                     xRadius:kCornerRadius -
      1 yRadius:kCornerRadius - 1];
