...
1) **Register drag types**: In `CleanList` table setup, register `NSPasteboard.PasteboardType.fileURL` (or UTType.fileURL) for drag-out.
2) **Provide pasteboard writers**: Implement `tableView(_:pasteboardWriterForRow:)` (or `tableView(_:writeRowsWith:to:)`) to return writers that expose absolute file URLs for the row. Support multiple selected rows.
3) **Resolve file URLs**: From `CleanList.Coordinator`, use a resolver provided by `CleanPanelController` (closure/delegate) that maps repo-relative paths to absolute URLs via the repository.
4) **Drag source masks**: Implement `draggingSession(_:sourceOperationMaskFor:)` to allow `.copy` (and optionally `.move` if appropriate). Use table-provided drag images; ensure selection is preserved on drag start.
5) **Handle clicked vs. selection**: If the drag begins on an unselected row, treat that row as the drag set; otherwise use the current selection.
6) **Missing paths policy**: Decide whether to skip items that no longer exist on disk or include them (drop may fail). Document the choice.
