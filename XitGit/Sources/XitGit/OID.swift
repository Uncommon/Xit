...
public struct GitOID: Sendable
{
  ...
  public static let shaLength = 40
  
  public static func zero() -> GitOID
  {
  ...
  public func withUnsafeOID<T>(_ block: (UnsafePointer<git_oid>) throws -> T) rethrows
    -> T
  {
    try withUnsafePointer(to: oid, block)
  }

  public mutating func withUnsafeMutableOID<T>(
...
extension GitOID: Equatable
{
  static let oidSize = 20
  
  public static func == (lhs: GitOID, rhs: GitOID) -> Bool
  {
    lhs.withUnsafeOID {
      (leftOID) in
      rhs.withUnsafeOID {
