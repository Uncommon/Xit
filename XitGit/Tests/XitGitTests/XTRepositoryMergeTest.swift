...
class XTRepositoryMergeTest: XTTest
{
  ...
  {
    return repository.stagingChanges.isEmpty &&
           repository.unstagedChanges().isEmpty
  }
  
  func assertWorkspaceContent(staged: [String], unstaged: [String],
                              file: StaticString = #file, line: UInt = #line)
  {
    XCTAssertEqual(repository.stagingChanges.map { $0.path }, staged,
                   "staged", file: file, line: line)
    XCTAssertEqual(repository.unstagedChanges().map { $0.path }, unstaged,
